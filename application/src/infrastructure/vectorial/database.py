...
class VectorialDataBase:
    ...
    def set_clientdb(self, flush=False):
        ...
        if isinstance(self.client, chromadb.ClientAPI):
            ...
            self.collection = self.client.get_or_create_collection(
                self.coll_name, embedding_function=self.ChromaLangchainEmbeddingFunction(self.embeddings)
            )
            # Create a Chroma clientdb using the initialized client, collection and embeddings.
            self.clientdb = Chroma(
                client=self.client,
                collection_name=self.coll_name,
                embedding_function=self.ChromaLangchainEmbeddingFunction(self.embeddings),
            )
    ...
    def get_retriever(self, words_number=2) -> VectorStoreRetriever:
        ...
        return self.clientdb.as_retriever(search_kwargs={"k": words_number, "fetch_k": 5}, search_type="mmr")
