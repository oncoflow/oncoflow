...
docpdf = fitz.open('data\pdfs\RCP_06052024.pdf')  # the file with the text you want to change
...
for page in docpdf:
    ...
    for identifier in classicalidentifiers:
        ...
        # found = page.get_text(identifier)  # list of rectangles where to replace
        for item in found:
            print(type(item))
    ...
    for ent in doctext.ents:
        ...
        for item in found:
            ...
            page.add_redact_annot(item, '' )  # create redaction for text
            page.apply_redactions()  # apply the redaction now
            # print(search_term, ent.label_, get_pseudonymized_item(search_term, ent.label_))
            page.insert_text(item.bl - (0, 2), text=get_pseudonymized_item(search_term, ent.label_), fontsize=9.29688, fontname='helv')
            # page.insert_text(item.bl, text=ent.label_, fontsize=9.29688  )
...
#         page.add_redact_annot(item, '')  # create redaction for text
#         page.apply_redactions()  # apply the redaction now
#         page.insert_text(item.bl - (0, 3), "HOP")
