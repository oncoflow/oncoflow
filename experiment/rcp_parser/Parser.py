...
class PDFParser:
    ...
    def extract_info(self, all_text):
        ...
        gastroscopie_absent_pattern = (
            r"G\s*a\s*s\s*t\s*r\s*o\s*s\s*c\s*o\s*p\s*i\s*e\s*:\s*(☑\s* \s*n\s*o\s*n)"
        )
        ...
        number_of_cures_pattern = (
            r"N\s*o\s*m\s*b\s*r\s*e\s* \s*d\s*e\s* \s*c\s*u\s*r\s*e\s*s\s*:\s*(\d+)"
        )
        ...
        location_of_intervention_pattern = (
            r"L\s*o\s*c\s*a\s*l\s*i\s*s\s*a\s*t\s*i\s*o\s*n\s*:\s*([\w\s]+)"
        )
        size_of_tumor_pattern = (
            r"T\s*a\s*i\s*l\s*l\s*e\s*:\s*([\d\s]*\.\s*[\d\s]+)\s* \s*c\s*m"
        )
        ...
        albumin_level_pattern = (
            r"A\s*l\s*b\s*u\s*m\s*i\s*n\s*e\s*:\s*([\d\s]*\.\s*[\d\s]+)\s* \s*g\s*/\s*L"
        )
        ...
        factor_v_level_pattern = (
            r"F\s*a\s*c\s*t\s*e\s*u\s*r\s* \s*V\s*:\s*([\d\s]*\.\s*[\d\s]+)%"
        )
        ...
        encephalopathy_diagnosis_date_match = re.search(
            encephalopathy_diagnosis_date_pattern, all_text
        )
        alcohol_consumption_current_match = re.search(
            alcohol_consumption_current_pattern, all_text
        )
        ...
        last_intervention_date_match = re.search(
            last_intervention_date_pattern, all_text
        )
        location_of_intervention_match = re.search(
            location_of_intervention_pattern, all_text
        )
        ...
        alpha_foetoprotein_level_match = re.search(
            alpha_foetoprotein_level_pattern, all_text
        )

        # Extracting and cleaning up the results
        date_rcp = date_rcp_match.group(1).replace(" ", "") if date_rcp_match else None
        nom_responsable = (
            " ".join(nom_responsable_match.group(1).split())
            if nom_responsable_match
            else None
        )
        sexe = sexe_match.group(1) if sexe_match else None
        date_naissance = (
            date_naissance_match.group(1).replace(" ", "")
            if date_naissance_match
            else None
        )
        dossier_passe_RCP = (
            dossier_passe_RCP_match.group(1) if dossier_passe_RCP_match else None
        )
        cirrhose_documentee = (
            cirrhose_documentee_match.group(1).strip()
            if cirrhose_documentee_match
            else None
        )
        gastroscopie_date = (
            gastroscopie_date_match.group(1).replace(" ", "")
            if gastroscopie_date_match
            else None
        )
        gastroscopie_absent = (
            gastroscopie_absent_match.group(1) if gastroscopie_absent_match else None
        )
        varices_present = (
            varices_present_match.group(1).strip() if varices_present_match else None
        )
        ascite_level = (
            ascite_level_match.group(1).strip() if ascite_level_match else None
        )
        encephalopathy_diagnosis_date = (
            encephalopathy_diagnosis_date_match.group(1).replace(" ", "")
            if encephalopathy_diagnosis_date_match
            else None
        )
        alcohol_consumption_current = (
            alcohol_consumption_current_match.group(1).replace(" ", "")
            if alcohol_consumption_current_match
            else None
        )
        treatment_type = (
            treatment_type_match.group(1).strip() if treatment_type_match else None
        )
        number_of_cures = (
            number_of_cures_match.group(1) if number_of_cures_match else None
        )
        last_intervention_date = (
            last_intervention_date_match.group(1).replace(" ", "")
            if last_intervention_date_match
            else None
        )
        location_of_intervention = (
            location_of_intervention_match.group(1)
            if location_of_intervention_match
            else None
        )
        size_of_tumor = (
            size_of_tumor_match.group(1).strip() if size_of_tumor_match else None
        )
        additional_info = (
            additional_info_match.group(1).strip() if additional_info_match else None
        )
        bilirubine_total = (
            bilirubine_total_match.group(1).strip() if bilirubine_total_match else None
        )
        creatinine_level = (
            creatinine_level_match.group(1).strip() if creatinine_level_match else None
        )
        albumin_level = (
            albumin_level_match.group(1).strip() if albumin_level_match else None
        )
        platelets_count = (
            platelets_count_match.group(1).strip() if platelets_count_match else None
        )
        prothrombin_time = (
            prothrombin_time_match.group(1).strip() if prothrombin_time_match else None
        )
        factor_v_level = (
            factor_v_level_match.group(1).strip() if factor_v_level_match else None
        )
        alpha_foetoprotein_level = (
            alpha_foetoprotein_level_match.group(1).strip()
            if alpha_foetoprotein_level_match
            else None
        )
        ...
        return [
            {
                ...
                "imc": {"IMC": "float", "ECOG": "float"},
                "Donnees_Biologique_moins_de_4_semaines": {
                    "Bilirubine_totale": {"value": bilirubine_total, "unit": "µmol/L"},
                    "Creatinine": {"value": creatinine_level, "unit": "µmol/L"},
                    "Albumine": {"value": albumin_level, "unit": "g/L"},
                    "Plaquettes": {"value": platelets_count, "unit": "G/L"},
                    "TP": {"pourcentage": prothrombin_time, "AVK_ou_AOD": "bool"},
                    "Facteur_V": {"value": factor_v_level, "unit": "%"},
                    "Alpha_foetoproteine": {
                ...
                "Decision_prise_en_charge": {
                    ...
                    "Exerese_Chirurgie": {"value": "bool", "precision": "string"},
